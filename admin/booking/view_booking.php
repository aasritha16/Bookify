...
// Debugging output
error_log("Debug: facility_code = $facility_code, name = $name, category = $category, ref_code = $ref_code, date_from = $date_from, date_to = $date_to, status = $status");
?>
<style>
    ...
    $(function() {
        $('.update_booking').click(function() {
            ...
            if (status == 1)
                ...
                action = "Cancel";
            _conf("Are you sure to " + action + " this facility booking [Ref. Code: <b><?= htmlspecialchars($ref_code) ?></b>]?", "update_booking", ["<?= htmlspecialchars($id) ?>", status]);
        });
    });
